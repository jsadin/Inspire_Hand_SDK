...
public:
     ...
     * @param device_protocol_config_path device_protocol_config.yaml 路径（用于从 protocol.type 推导
     * interfaces_profile）
     * @return 设备节点配置列表（按设备分组）
     */
    static std::vector<DeviceNodeConfig> loadDeviceNodeConfigs(const std::string& config_path,
     ...
     */
    static std::shared_ptr<RegisterController>
    createDeviceNode(const DeviceNodeConfig& config, DeviceManager& device_manager,
     ...
     */
    static std::vector<std::shared_ptr<RegisterController>>
    createAllDeviceNodes(const std::string& config_path, DeviceManager& device_manager,
                         const std::unordered_map<std::string, std::shared_ptr<Protocol>>& device_protocols,
                         const std::string& device_protocol_config_path);
