...
std::vector<uint8_t> EG2_4C2_serial_can_Protocol::buildSerialCanFrame(uint32_t can_id,
    ...
    const uint8_t valid_len =
        is_read ? static_cast<uint8_t>(0x01) : static_cast<uint8_t>(std::min(payload.size(), kCanPayloadLength));
...
bool EG2_4C2_serial_can_Protocol::parseAndValidateFrame(const std::vector<uint8_t>& frame, uint8_t expected_op_type,
...
std::vector<uint8_t> EG2_4C2_serial_can_Protocol::buildWriteCommand(int address, const std::vector<int>& values) {
...
IoError EG2_4C2_serial_can_Protocol::writeRegister(Device device, const std::string& reg_name,
    ...
    if (values.size() > max_count) {
        logger->error("[EG2-4C2] 写寄存器 {} 超出单帧上限: 输入 {} 个, 上限 {} 个", reg_name, values.size(), max_count);
        return IoError::InvalidArgument;
    }
    ...
    try {
        logger->info("[EG2-4C2] 写寄存器 {} addr=0x{:04X} can_id=0x{:08X} values=({}) tx={}", reg_name, address,
                     buildCanId(kOpWrite, address), valuesToText(values), bytesToHex(cmd));
...
RegisterReadResult EG2_4C2_serial_can_Protocol::readRegister(Device device, RingBuffer& ringBuffer,
    ...
    while (byte_offset < target_len) {
        ...
        logger->debug("[EG2-4C2] 读寄存器 {} addr=0x{:04X} can_id=0x{:08X} len={} tx={}", reg_name, frame_addr, can_id,
        ...
        const auto response = readOneFrameRaw(device, 30);
        if (response.empty()) {
            logger->error("[EG2-4C2] 读寄存器 {} 超时无回包（timeout），addr=0x{:04X} len={}", reg_name, frame_addr,
                          chunk_len);
            return {IoError::Timeout, {}};
        }
...
std::pair<bool, TouchDataResult> EG2_4C2_serial_can_Protocol::parseTouchData(RingBuffer&, int) { return {false, {}}; }
