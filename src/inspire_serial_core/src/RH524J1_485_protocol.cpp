...
IoError RH524J1_485_Protocol::writeRegister(Device device, const std::string& reg_name,
    ...
    if (reg_name == "angleSet") {
        try {
            ...
            if (explicit_count == 1) {
                ...
                logger->info("[RH524J1] angleSet 单关节写入 ID{} 下标{} 地址0x{:04X} 值={}", drive_id, explicit_index,
                             address, write_values[0]);
            } else {
...
RegisterReadResult RH524J1_485_Protocol::readRegister(Device device, RingBuffer& ringBuffer,
                                                      const std::string& reg_name, size_t length) {
