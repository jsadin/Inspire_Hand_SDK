...
std::vector<uint8_t> buildExpectedSerialCanFrame(uint32_t can_id, const std::vector<uint8_t>& payload8,
                                                 uint8_t payload_len_field, bool is_read) {
    (void)is_read;
...
TEST(EG2_4C2SerialCanProtocol, RemoveA5Escape) {
    ...
    std::vector<uint8_t> expected = {0x01, 0x80, 0xAA, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00,
                                     0x00, 0x00, 0x01, 0x00, 0x01, 0x00, 0xD4, 0x55, 0x55};
